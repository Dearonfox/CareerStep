...
> 작성일: 2026.06.20 · 대상 모듈: `ai-api`, `main-api`, `frontend`
...
[main-api]
   ...
   3) 후보 공고 요약 + 프로필을 ai-api로 전달 (X-Internal-Key)
   │  POST /recommend/jobs
   ▼
...
1. **Vector DB 도입**: 공고 요약을 임베딩해 후보 검색을 키워드/직군 매칭 → 의미 유사도로 고도화
2. **Neo4j 합격 후기 그래프**: 합격자 스펙↔활동 관계 데이터 수집 후, "비슷한 합격자가 한 활동" 추천으로 Graph RAG 완성
3. **마일스톤 생성기 분리**: 본 매칭의 `gaps` 를 입력으로 주차별 액션 플랜 생성 (Claude 분기)

본 설계의 후보 검색 단계는 이후 Vector DB/Neo4j 검색기로 **교체 가능하도록 인터페이스 분리**해 둔다 (retrieval 함수를 추상화).

---

## 10. 작업 분해 (다음 세션 To-Do)

우선순위 순:

1. (ai-api) `schemas.py` — `JobInput`/`RecommendedJob` 을 MongoDB 요약 구조로 보강
2. (ai-api) `prompts.py` — 매칭 점수 기준·근거 연결 규칙 추가
3. (main-api) 후보 검색 모듈 신설 — `routed_roles` + skills 1차 스코어링, Top-K 선별
4. (main-api) `GET /api/recommendations/me` 라우트 + ai_proxy 연동
5. (ai-api) `recommendations.py` 실데이터 연결 + 모델 라우팅(mini/roadmap 분기)
6. (frontend) `useRecommendStore` 실연동, MatchScoreBadge/JobCard 바인딩
7. (test) 골든셋 프로필 2~3개로 매칭 품질·비용 검증 (pytest)
